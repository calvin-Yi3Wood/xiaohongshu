...
我将为您创建一个实用的小红书文案优化应用程序的原型。这个应用将能根据您的需求，自动生成符合小红书算法推荐规则的高质量文案。

由于我无法提供实际的可执行应用程序，我将提供一个交互式的文本界面设计，您可以将其作为参考来实现一个实际的应用。
...
让我为您提供一个可以直接使用的交互式脚本，它基于我们之前的分析和算法，可以生成符合小红书推荐系统偏好的文案内容。
...
class XiaohongshuContentGenerator:
    ...
    def generate_random_time(self):
        ...
        units = ["天", "个月", "年"]
        if units[0]: # 天
            return f"{random.randint(21, 100)}{units[0]}"
        elif units[1]: # 月
            return f"{random.randint(1, 24)}{units[1]}"
        else: # 年
            return f"{random.randint(1, 5)}{units[2]}"
    ...
    def generate_full_content(self, problem_type, recipe_types=None, blogger_name=None):
        ...
        outro = outro_template.replace("{饮用时间}", self.random_pick(self.drinking_times)) \
                             ...
                             .replace("{注意事项1}", self.random_pick(self.precautions)) \
                             .replace("{注意事项2}", self.random_pick([p for p in self.precautions if p != outro.replace("{注意事项1}", "")])) \
                             .replace("{效果1}", self.random_pick(self.effect_templates)) \
                             .replace("{效果2}", self.random_pick([e for e in self.effect_templates if e != "{效果1}"])) \
                             .replace("{效果3}", self.random_pick([e for e in self.effect_templates if e not in ["{效果1}", "{效果2}"]])) \
